...
import com.technology.ncode.GenerateDocumentation.GenerateDocumentationFactoryContent;
...
// Add the inputPanelContainer to the main panel (bottom section)
    ...
    public void setSelectedCode(String code) {
        ...
        if (code != null && !code.isEmpty()) {
            ...
            new Thread(() -> {
                ...
                String testCase = VertexAIChatbot.getVertexAIResponse(prompt);
            ...
            //appendUserMessage("No code selected. You can still enter your query.", true);
            enableInput();
        }
    ...
    private void appendUserMessage(String text, boolean isNewChat) {
        SwingUtilities.invokeLater(() -> {
            ...
            // Append previously selected code if it's a new chat and lastSelectedCode exists
            if (isNewChat && !lastSelectedCode.isEmpty()) {
                message.append("<pre style='color:gray; background:#282828; padding:10px; font-family:Courier New; font-size:12px; white-space:pre-wrap; word-wrap:break-word;'>")
                        .append(lastSelectedCode.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")) // Escape HTML characters
                        .append("</pre><br>");
            }
